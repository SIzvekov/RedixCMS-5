<?if(!$this->thishomepage){?><h1><?=PAGE_NAME?></h1><?}else{
...
foreach($array as $item)
{
	...
	{
		$line++;
		echo $line<3?'<div class="row"><ul class="list">':'<div class="row2"><div class="row-holder"><ul class="list">';
		$startedline = 1;
	}
	?>
<li>
		...
		<div class="description" style="padding:0px 5px;width:200px;overflow:hidden;">
			<div class="holder">
				<div class="frame" style="padding:5px 0;">
					<strong class="title" style="line-height:18px;"><a href="<?=$item['url']?>"><?=$item['title']?></a></strong>
				</div>
			</div>
		</div>
	</div>
